export default class GameObjects {
  ...
  createNewCoinLocations( groundObstacleLocations: any[] ){
    ...
    for ( let i = 0; i < 6; i++ ) { trackOneX.push( i ); }
    for ( let i = -3; i > -60; i--) { trackOneZ.push( i ); }
  ...
  updateMovingPlatforms( delta: number, movingPlatforms: { name: string, space: string, direction: number, speed: number, position:{ x: number, y: number, z: number }}[]) {
    const verticalMin = -5;
    const verticalMax = 5;
